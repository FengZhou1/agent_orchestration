...
图中 LLM 节点标注条件平均输入和输出 token 数 `(P, O)`，无状态服务节点标注低负载平均处理时间。所有数值均为理论仿真的参考配置，用于覆盖短请求、工具密集请求、并行研究请求和长上下文编码请求，不作为生产系统的逐请求测量结果。
...
递增，其中 `D` 为当前阶段各无状态服务返回内容折算的 token 数，`A` 为下一阶段新增的角色指令、工具描述或格式化开销。并行分支汇聚时，`D` 和分支 LLM 输出取所有已完成分支之和。图中节点数值为按该递推关系得到的条件平均值。
...
入口 LLM 在直接回答、稠密检索和混合搜索之间进行概率选择，概率分别为 0.15、0.55 和 0.30。直接回答分支由输入、输出 token 均值为 $(140,400)$ 的生成节点处理，用于表示短输入、长输出请求；两类检索结果由证据整合 LLM 处理，并进入检索增强生成 LLM。该图形成直接生成、稠密检索增强和混合检索增强三种 pattern flows。
